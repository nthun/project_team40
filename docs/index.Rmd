...
Using all information from the exploratory analysis, we create the final dataset.
...
Apart from heteroscedasticity, it seems like there is a strange separation in the predictor levels. After some investigation, it seems like all residuals on the left hand side come from the Japanese sample. 
  We need to decide where this comes from, and come up with a plan to handle it. 
...
```{r}
h2 <- 
  lmer(wb_overall_mean ~ scale(religiosity) * scale(cnorm_mean) + 
         # personal level confounders
                         age + gender + ses + education + denom_lump +
...
# Calculate BIC based Bayes factor for 
# H0 vs H1
exp((BIC(h0) - BIC(h1))/2)
# H1 vs H2
exp((BIC(h1) - BIC(h2))/2)
...
b1 <- 
  brm(wb_overall_mean ~ religiosity  + 
         # personal level confounders
                         age + gender + ses + education + denom_lump +
         # country level confounders
                         gdp_scaled + sample_type + 
...
```{r eval=FALSE, cache=FALSE, include = FALSE}
b2 <- 
  brm(wb_overall_mean ~ religiosity * cnorm_mean + 
      # personal level confounders
                         age + gender + ses + education + denom_lump +
      # country level confounders
                         gdp_scaled + sample_type + 
